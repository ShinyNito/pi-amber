import { estimateTextTokens } from "./tokenLedger.ts";
...
/** Extract paths/commands etc. from the most recent messages; if the summary
 * keeps none of them, treat it as hallucinated loss and fail validation. */
export function buildVerificationSignals(recent: VerificationSource[]): string[] {
  const out: string[] = [];
  const seen = new Set<string>();

  for (const message of recent) {
    if (message.content) extractVerificationSignalsFromText(message.content, out, seen);
    if (message.text) extractVerificationSignalsFromText(message.text, out, seen);
    if (message.details) extractVerificationSignalsFromText(message.details, out, seen);
...
export function validateCompactionSummary(
  ...
  // CJK-aware short check against the RAW output (XML structure included): a
  // structurally complete summary has a natural minimum size, while a hollow
  // shell (bare tags) falls far below it. Estimating only the tag contents
  // would misjudge dense summaries as "too short".
  const totalTokens = estimateTextTokens(raw);
